# Task Template: Medical Imaging Report Generator

## Overview
Radiology assistant running on Victron hardware for report generation

## What To Do

### Objective
Generate radiology reports from imaging findings and clinical context

### Data Requirements
- **Data Sources**: Radiology reports (MIMIC-CXR), imaging findings databases, radiology guidelines
- **Data Format**: JSON with imaging_findings, clinical_context, generated_report, confidence_scores
- **Data Volume**: 30,000+ report examples
- **Privacy Considerations**: De-identified imaging data

### Implementation Steps
1. Collect and preprocess training data from Radiology reports (MIMIC-CXR), imaging findings databases, radiology guidelines
2. Set up base model from Hugging Face (see section below)
3. Configure fine-tuning parameters for Medical Imaging Report Generator
...
!lm-evaluation --model hf --model_args pretrained=./results --tasks Medical Imaging Report Generator --device cuda
```
...
### Success Criteria
- **Baseline Performance**: Report quality score > 0.7
- **Edge Compatibility**: Latency < 3sreport, Memory < 4GB
- **Task Accuracy**: Report quality > 0.85, clinical accuracy > 82%
- **Human Preference**: >72% baseline generators preference over baseline
...
- 10_medical_imaging_report_generator
---

# Model Card for 10_medical_imaging_report_generator
...
## Citation
```bibtex
@misc{10_medical_imaging_report_generator,
  title={10_medical_imaging_report_generator},
  author={QuantumIndSSI Ltd},
  year={2026},
  publisher={Hugging Face},
  howpublished={\url{https://huggingface.co/[USERNAME]/10_medical_imaging_report_generator}}
}
```
```
...
# Create repository
api.create_repo(
    repo_id="quantumindssi/10_medical_imaging_report_generator",
    repo_type="model",
...
### 5. Upload via CLI
```bash
huggingface-cli repo create quantumindssi/10_medical_imaging_report_generator --type model
huggingface-cli upload quantumindssi/10_medical_imaging_report_generator ./results/checkpoint-final
```
...
# Then upload
huggingface-cli upload quantumindssi/10_medical_imaging_report_generator README.md
```
...
git init
git add .
git commit -m "Initial commit: 10_medical_imaging_report_generator fine-tuning"
git remote add origin https://github.com/quantumindssi/10_medical_imaging_report_generator
...
st.title("10_medical_imaging_report_generator Demo")
st.sidebar.write("Model: quantumindssi/10_medical_imaging_report_generator")

pipe = pipeline("text-generation", model="quantumindssi/10_medical_imaging_report_generator")
...
```bash
# Create Space
huggingface-cli space new quantumindssi-10_medical_imaging_report_generator-demo --sdk streamlit

# Deploy demo
cd quantumindssi-10_medical_imaging_report_generator-demo
...
#### Tutorial Series
1. "Getting Started with 10_medical_imaging_report_generator"
...
## Model Authors
- 10_medical_imaging_report_generator: @[author]
- 10_medical_imaging_report_generator: @[author]
```
...
def announce_model(model_name, model_link):
    ...
    A fine-tuned SLM for Healthcare & Medical AI
